...
log_info "Gateway port: $GATEWAY_PORT (external), $INTERNAL_PORT (internal)"
...
case "$UPSTREAM" in
    openclaw)
        GATEWAY_CMD="/usr/local/bin/$CLI_NAME gateway --port ${INTERNAL_PORT} --bind loopback"
        ;;
    picoclaw)
        GATEWAY_CMD="/usr/local/bin/$CLI_NAME gateway --port ${INTERNAL_PORT}"
        ;;
    zeroclaw)
        GATEWAY_CMD="/usr/local/bin/$CLI_NAME gateway --port ${INTERNAL_PORT}"
        ;;
...
log_info "Web interface available at: http://localhost:$GATEWAY_PORT"
