...
if [ "$(id -u)" = "0" ] && id openclaw >/dev/null 2>&1; then
    ...
    # Use su -c with properly quoted command
    exec su -s /bin/bash openclaw -c 'cd /data && HOME=/data/.openclaw OPENCLAW_STATE_DIR=/data/.opencl exec /usr/local/bin/openclaw.real "$@"'
    ...
    exec su -s /bin/bash openclaw -c 'cd /data && HOME=/data/.openclaw OPENCLAW_STATE_DIR=/data/.openclaw exec /usr/local/bin/openclaw.real "$@"'
else
